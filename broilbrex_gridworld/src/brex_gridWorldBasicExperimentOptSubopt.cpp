...
int main() 
{
    ...
    int skip = 5;
    auto birl_total=0.0000,brex_total = 0.0000;
    int startSeed = 132;
    double eps = 0.001;
...
for(unsigned int rolloutLength : rolloutLengths)
{
    ...
    for(double alpha : alphas)
    {
		...
		auto birl_start = std::chrono::high_resolution_clock::now();
		...
		auto birl_stop = std::chrono::high_resolution_clock::now();
		auto birl_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(birl_stop - birl_start);
		birl_total += birl_elapsed.count()*1e-9;
		cout << "111111111111111111111111111111Time: birl=" << birl_elapsed.count()*1e-9 << " ; " <<":"<< endl;
                /// We use the Map Policy as the evaluation policy

                
                ///Run Bayesian REX to compare
		auto brex_start = std::chrono::high_resolution_clock::now();
		...
		auto brex_stop = std::chrono::high_resolution_clock::now();
		auto brex_elapsed =  std::chrono::duration_cast<std::chrono::nanoseconds>(brex_stop - brex_start);
		brex_total += brex_elapsed.count()*1e-9;
		cout << "111111111111111111111111111111Time: brex=" << brex_elapsed.count()*1e-9 << " ; " <<  ":"  << endl;
		cout << "Time: birl=" << birl_total << ", brex=" << brex_total << endl;
                if(rep == 0)
                    outfile << "#true policy loss value birl, true policy loss value diff brex, birl time , brex time" << endl;
                outfile << birltrueDiff << ", " << brextrueDiff << "," <<birl_total << ","<< brex_total << endl;
